...
NSMutableDictionary* allUserMessages;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //Example title
    self.navigationItem.title = self.messageName;
}

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    int num = (int)[[allUserMessages valueForKey:self.messageName] count];
    if(num != 0){
        self.allMessages = [allUserMessages valueForKey:self.messageName];
    }else{
        NSLog(@"count zero");
...
- (UIView*)createMessageWithText: (NSString*)text Image: (UIImage*)image DateTime: (NSString*)dateTimeString isReceived: (BOOL)isReceived
{
    ...
    chatBubbleView.backgroundColor = isReceived? [UIColor grayColor]:[UIColor blueColor];
    ...
    chatBubbleContentView.backgroundColor = isReceived? [UIColor grayColor]:[UIColor blueColor];
    ...
    if(isReceived){
        ...
        arrowIV.image = [arrowIV.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        [arrowIV setTintColor:[UIColor blueColor]];
    }
    ...
    if(allUserMessages == nil){
    allUserMessages = [[NSMutableDictionary alloc] init];
    }
    [allUserMessages setValue:self.allMessages forKey:self.messageName];
    NSMutableArray* arr = [allUserMessages valueForKey:self.messageName];
    NSLog(@"ARR IS %@",arr);
//    allUserMessages = self.allMessages;
    return outerView;
}
